...
class AppointmentLocalRepository implements IAppointmentRepository {
  ...
  AppointmentLocalRepository({required AppointmentLocalDataSource appointmentLocalDataSource})
  ...
  Future<Either<Failure, void>> deleteAppointment(String appointmentId, String? token) async {
    try {
      ...
      return Left(LocalDatabaseFailure(message: "Error deleting appointment: $e"));
    }
  }
  ...
  Future<Either<Failure, AppointmentEntity>> getAppointmentById(String appointmentId, String? token) async {
    try {
      final appointmentEntity =
          await _appointmentLocalDataSource.getAppointmentById(appointmentId, token);
  ...
  Future<Either<Failure, void>> saveAppointment(AppointmentEntity appointmentEntity, String? token) {
    try {
      _appointmentLocalDataSource.saveAppointment(appointmentEntity, token);
  ...
  Future<Either<Failure, void>> updateAppointment(String appointmentId, AppointmentEntity updatedAppointment, String? token) async {
    try {
      await _appointmentLocalDataSource.updateAppointment(appointmentId, updatedAppointment, token);
      return const Right(null);
    } catch (e) {
      return Future.value(Left(LocalDatabaseFailure(message: e.toString())));
    }
